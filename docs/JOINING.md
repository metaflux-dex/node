...
key, and the host:port set you will listen on. The operators will tell you which
upgrade window will carry your entry.
...
The chain was re-genesised on **2026-07-26** with the same `chain_id` (114514)
and a new genesis. **That can happen again.** A re-genesis invalidates every
...
```
db1237bb365202c4b34bc60bff7511e605ef0f25fff149f5ceb84cc439f57314
```

The hash is taken over the file's BYTES. Do not reformat, re-indent or re-key
`genesis.json` — any byte change moves the hash, and the visor then halts your
node for reporting the wrong chain identity.
...
The seed peers are already filled in, with the mandatory `pubkey_hex` on each.
...
Measured duration: about 5 minutes. You do nothing. Never add 80 to
`RestartPreventExitStatus` — that blocks the node's own recovery path.
...
past the freeze. It exits 78, and that needs an operator.

## Measuring the chain, not assuming it

Block cadence is **measured, never fixed**. It moves. To measure it, sample the
committed round twice and divide by the elapsed seconds:
...
Take the `round` field, wait a known interval, take it again. Never size anything
on a configured interval — `target_block_interval_ms` is a target, not the
observed rate.
